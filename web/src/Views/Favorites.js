...
import { getProperties, getUser } from '../Utils/functions';
    ...
    useEffect(() => {
        getUser(getLoggedUserId()).then((data) => {
            ...
            getProperties().then((propertylist) => {
                setProperties(filter(propertylist, data));
            });
        });
    }, []);
    return(
        <div className="alt-grid-fix bg-secondary" id="gridDiv">
            <nav className="navbar navbar-expand-sm position-fixed fl-right sub-nav">
                <div className="card left-padding right-padding bg-secondary navbar-collapse w-100 order-3 dual-collapse2">
                    <ul className="navbar-nav ml-auto">
                        ...
                        <li>
                            <button className="nav-link btn btn-dark text-light btn-margin">
            ...
            <div className="p-t-sub" id="Grid">
                {properties && user ? <Grid properties={properties}/> : <Container><br/><br/><br/>Aún no se han agregado favoritos a la lista.</Container>}
            </div>
        </div>
    );
}

function filter(properties, user){
    if(!user || !user.favorites) return null;
    const favorites = user.favorites;
    var filteredProperties = [];
    favorites.map((favorite)=>{
        filteredProperties.push(properties.find(element => element._id == favorite.id));
    })
